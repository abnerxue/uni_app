<template>
	<view class="backgrounds">
		...
		<view class="swiper-vido">
			...
			<view class="currenttime">据该商品结束还剩：<uni-countdown :minute="0" splitorColor="white" backgroundColor="#C2060C" color="white" :second="0" /></view>
			<view class="currentactive" v-if="id==1"><text>限时团购</text></view>
			<view class="currentactive" v-if="id==2"><text>限时秒杀</text></view>
		</view>
		<!-- 标题 价格 -->
		<view class="title-money">
			<view class="price" v-if="id==1">￥{{ group.price }} <text class="yuanjia">￥{{ goods.price }} </text></view>
			<view class="price" v-if="id==2">￥{{ seckill.price }} <text class="yuanjia">￥{{ goods.price }} </text></view>
		...
		<!-- 发起团购列表 -->
		<view class="group" v-if="id==1">
			<view class="group-title">
				<view>他们都在拼，可直接参团</view>
				<view></view>
			</view>
			<view class="content-group" v-for="group_list_ in group_list_">
				<view class="head-image-name">
					<image class="head-img" :src="group_list_.avatar"></image>
					<view class="group-nam">
						<view class="group-name">{{group_list_.userName}}</view>
						<view class="group-names">还差{{group_list_.goodsNum - group_list_.joinNum}}人成团,剩余 <uni-countdown class="example-body" :minute="0" splitorColor="#9A9A9A" color="#9A9A9A" :second="group_list_.timeRemaining" /> 结束</view>
					</view>
				</view>
				<view class="btn-group" @click="participate(group_list_.id)">去参团</view>
			</view>
		</view>
		<!-- 评论 -->
		<view class="Comment">
		...
		<view class="shangpinguige" v-show="guige==true">
			<view class="shangpin">
				<view class="shangpinguige-title">
					...
					<view class="coloe" @click="guige=false">X</view>
				</view>
		...
		<view class="bottom">
			...
			<view class="rels">
				<view class="addcart" @click="buy(1)">
					<view>￥{{goods.price}}</view>
					<view>单独购买</view>
				</view>
				<view class="buy" @click="buy(2,1)" v-if="id==1">
					<view>￥{{group.price}}</view>
					<view>发起拼团</view>
				</view>
				<view class="buy" @click="buy(2,2)" v-if="id==2">
					<view>￥{{seckill.price}}</view>
					<view>秒杀</view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			that.id = option.id;
			if (that.id == 1) {
				that.url = '/api/group/detail';
				// 获取发起团购的人数
		
				server.getJSON(
					'/api/group/initiate_list', {
						ajax: true,
						groupId: that.activityId,
					},
					function(res) {
						console.log(res)
						that.group_list_ = res.data.list;
					}
				);
			} else if (that.id == 2) {
				that.url = '/api/seckill/detail';
			} else if (that.id == 3) {
				that.url = '/api/score/detail';
			}
			server.getJSON(
				that.url, {
					id: that.activityId
		...
		methods: {
			...
			// 结算
			buy(id, ids) { // id 是区分是否是直接购买   ids 区分是哪个活动
				let that = this;
				if (id == 1) {
					uni.navigateTo({
						url: '../order/confirm_order?number=1&gid=' + that.goods.gid
					})
				} else if (ids == 1) {
					uni.navigateTo({ //activityId 活动id
						url: '../order/confirm_order?number=1&gid=' + that.goods.gid + '&id=' + ids + '&activityId=' + that.group.id
					})
				} else if (ids == 2) {
					uni.navigateTo({ //activityId 活动id
						url: '../order/confirm_order?number=1&gid=' + that.goods.gid + '&id=' + ids + '&activityId=' + that.seckill.id
					})
				}
			},
			//购物车
			...
			participate(id){
				uni.navigateTo({
					url:'../user/group_list/group_del?id='+id+'&group_chanjia=1'
				})
			},
			// 推荐商品进入详情页
			del(gid){
